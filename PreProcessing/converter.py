...
class Converter():
    ...
    def convert(self,filepath,outpath,cellkey,cellnum,ret=True,bounds=None):
        ...
        for row in range(len(NASA_df)):
            if NASA_df.loc[row,"Cycle_Type"]=="discharge" and tp=="charge":
                tp="discharge"#each cycle is either discharge or charge
                cycle+=1
        ...
        #cycles should now look like 1:charge, 2:discharge, 3: charge, etc.
        ...
        if bounds:
            if bounds=="Calculate":
                lower_bound=NASA_df.groupby('Cycle')['Voltage'].min().max()#if voltage bounds are automatically calculated we take
                upper_bound=NASA_df.groupby('Cycle')['Voltage'].max().min()#infimum and supremum
            else:
        ...
        cap_loss=0
        discharge=0
        val=0# values are kept to track outlier detection
        mean=0
        ref=0
        ...
        NASA_df["Instantaneous_Capacity"] = 0
        NASA_df["Capacity_Loss"]=0
        
        NASA_df["Reference Capacity"] =0
        for cycle in NASA_df["Cycle"].unique():
            cycle_indices=NASA_df.index[(NASA_df["Cycle"]==cycle)]# get only the part of df in the given cycle
            cycle_df=NASA_df.loc[cycle_indices,["Cycle_Type","Current","Cycle Time","Voltage"]]
            
            name="charge"
            if cycle_df.loc[cycle_indices[0],"Cycle_Type"]=="discharge":# detect discharge cycle
                name="discharge"
                voltmin = cycle_df["Voltage"].idxmin()

            
            cycle_df["Instantaneous_Capacity"] = scipy.integrate.cumulative_trapezoid(cycle_df['Current'], cycle_df['Cycle Time'], initial=0)/3.6# calculate instantaneous capacity
            
            #plt.scatter(cycle_df["Cycle Time"],cycle_df["Instantaneous_Capacity"])
            #plt.title(f"Cycle {cycle} Instantaneous Capacity plot")
            #plt.show()
            NASA_df.loc[cycle_indices,"Instantaneous_Capacity"]=cycle_df["Instantaneous_Capacity"]# place instantaneous capacity in df
           
            new_ref=max(cycle_df["Instantaneous_Capacity"])-min(cycle_df["Instantaneous_Capacity"])# rudimentary ref_cap calculation
            
            # if bounds are specified/calculated then special ref cap is given
            if bounds:
                indices=NASA_df.index[(NASA_df["Cycle"]==cycle)&(NASA_df["Voltage"]>lower_bound)&(NASA_df["Voltage"]<upper_bound)]
                indices=indices[0:voltmin]
                new_ref=max(NASA_df.loc[indices,"Instantaneous_Capacity"])-min(NASA_df.loc[indices,"Instantaneous_Capacity"])
            mean=val*mean/(val+1)+new_ref/(val+1)#calculates mean ref cap from 0 to now
            val+=1
            if ref!=0 and abs(mean-new_ref)>500:#if obvious anomaly then just set to previous value
                new_ref=ref*.999
            if name=="discharge" and cycle not in NASA_df["Cycle"].unique()[0:2]:
                
                cap_loss=new_ref-discharge
                discharge=new_ref
            if abs(cap_loss)>600:
                cap_loss=0
            ref=new_ref
            print(f"Reference Capacity {name} Cycle {cycle}: {ref}")
            NASA_df.loc[cycle_indices,"Reference Capacity"]=ref# set ref cap into df
            NASA_df.loc[cycle_indices,"Capacity_Loss"]=cap_loss
        # In[72]:
        ...
        NASA_df = NASA_df.drop(columns=['tag','Capacity'])
